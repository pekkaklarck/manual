...
explicitly [create](../syntax/tasks.md#creating-tasks) and execute tasks. For most parts task execution
...
The generic automation mode can also be enabled by using the `--rpa`{.option}
option. In that case the executed files can have either tests or tasks.
Alternatively `--norpa`{.option} can be used to force the test automation
...
is a need to [select only certain tasks for execution](configuration.md#selecting-test-cases), `--task`{.option}
can be used instead of `--test`{.option}. Additionally the aforementioned
`--rpa`{.option} can be used to control the execution mode.
