...
tasks works the same way as [creating tests](tests.md#creating-tests) and the only real difference
is in terminology. Tasks can also be organized into [suites](suites.md#creating-test-suites) exactly like
test cases.

## Task syntax

Tasks are created based on the available keywords exactly like test cases,
and the task syntax is in general identical to the [test case syntax](tests.md#test-case-syntax).
